...
async function waitForSuiteOutcome(statusChip, logPane, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (normalizedStatusText.includes('FAILED') || normalizedStatusText.includes('CANCELLED')) {
      ...
      throw new Error(`SmokeStrategist did not complete successfully:\n${recentLines}`);
    }

    await new Promise((resolve) => setTimeout(resolve, 500));
  }

  throw new Error('Timed out waiting for SmokeStrategist full-suite completion.');
}
...
test('SmokeStrategist can be launched from App Config and complete a live strategist cycle', async ({ page, request }) => {
